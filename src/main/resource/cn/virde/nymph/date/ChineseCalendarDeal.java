...
private static int checkDay(int iYear, int iMonth, int iDay) { 
 int iMonthDays = ChineseCalendarDeal.iGetSYearMonthDays(iYear, iMonth); 
...
public Week toWeek() { 
 ...
 for (int i = 1901; i < iYear; i++) { 
   if (ChineseCalendarDeal.bIsSolarLeapYear(i)) 
     iOffsetDays += 366; 
   else 
     iOffsetDays += 365; 
 } 

 iOffsetDays += ChineseCalendarDeal.iGetSNewYearOffsetDays(iYear, iMonth, iDay); 
...
public LunarDate toLunarDate() { 
 ...
 LunarDate ld; 
 iDate = Integer.parseInt(ChineseCalendarDeal.sCalendarSolarToLundar(this.iYear, this.iMonth, this.iDay));
 iYear = iDate / 10000; 
...
private static int checkMonth(int iYear, int iMonth) { 
 if ((iMonth > 12) && (iMonth == ChineseCalendarDeal.iGetLLeapMonth(iYear) + 12)) { 
   return iMonth; 
 } else if (iMonth > 12) { 
   System.out.println("Month out of range, I think you want 12 "); 
   return 12; 
...
private static int checkDay(int iYear, int iMonth, int iDay) { 
 int iMonthDays = ChineseCalendarDeal.iGetLMonthDays(iYear, iMonth); 
...
public CnWeek toWeek() { 
 int iOffsetDays = 0; 
 for (int i = 1901; i < iYear; i++) 
   iOffsetDays += ChineseCalendarDeal.iGetLYearDays(i); 

 iOffsetDays += ChineseCalendarDeal.iGetLNewYearOffsetDays(iYear, iMonth, iDay); 
...
public SolarDate toSolarDate() { 
 int iYear, iMonth, iDay, iDate; 
 SolarDate sd; 
 iDate = Integer.parseInt(ChineseCalendarDeal.sCalendarLundarToSolar(this.iYear, this.iMonth, this.iDay));
 iYear = iDate / 10000; 
 iMonth = iDate % 10000 / 100; 
 iDay = iDate % 100; 
